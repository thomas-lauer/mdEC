...
- Fuer den Sprung von der Vorschau in den Markdown-Code wird beim Rendern jeder Block mit seiner Quell-Zeile (`data-source-line`) versehen. Da `marked.lexer` die Inline-Token (inkl. Referenz-Links) bereits aufloest, liefert das Rendern Token fuer Token korrektes HTML; die Quell-Zeile ergibt sich aus der Summe der Zeilenumbrueche in den `raw`-Texten der vorhergehenden Token (inkl. `space`-Token fuer Leerzeilen).
- Die Zuordnung wurde verfeinert: Aus den `items`- bzw. Zeilen-Strukturen der Token werden auch einzelne Listenpunkte (`<li>`, rekursiv fuer verschachtelte Listen) und Tabellenzeilen (`<tr>`) mit ihrer Quell-Zeile versehen. Bei Tabellen liegt zwischen Kopfzeile und erster Datenzeile die Trennzeile, weshalb Datenzeilen bei `startLine + 2` beginnen. Der Klick-Handler sucht das naechstgelegene Element mit `data-source-line`, sodass automatisch die feinste passende Stelle gewinnt.
- Um in der Textarea verlaesslich (auch bei Zeilenumbruch) zur Ziel-Zeile zu scrollen, wird der vertikale Pixel-Offset ueber ein unsichtbares Spiegel-Element mit identischen Umbruch-Stilen gemessen. Reines Setzen der Selektion scrollt die Textarea nicht zuverlaessig.
- Rechtsklick im Editor: `onContextMenu` mit `preventDefault` unterdrueckt das native Kontextmenue und wechselt stattdessen in die Vorschau.
